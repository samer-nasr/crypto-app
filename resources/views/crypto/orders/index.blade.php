                ...
                <table class="w-full text-left border border-gray-200 text-sm table-auto">
                    ...
                    <tbody>
                        @foreach ($orders as $order)
                            ...
                            <td class="p-2 {{ $order->type == 'Buy' ? 'text-green-600' : 'text-red-600' }} font-semibold">{{ $order->type }}</td>
                            <td class="p-2">{{ $order->coin->name }}</td>
                            <td class="p-2">${{ $order->price }}</td>
                            <td class="p-2">{{ $order->quantity }} {{ $order->coin->code }}</td>
                            <td class="p-2">${{ $order->counter_price }}</td>
                        </tr>
                        @endforeach
